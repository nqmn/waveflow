...
def load_features(path: Path) -> np.ndarray:
    """Extract FEATURE_COLUMNS rows (angle converted to radians) from the CSV dataset."""
...
def main():
    ...
    fit_start = time.time()
    print(f"  Input shape: {data.shape} ({data.shape[1]}D: {data.shape[1]-1} features + 1 label)")
    gmm = GaussianMixture(
