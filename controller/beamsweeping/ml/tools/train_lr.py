...
====================================
The training dataset uses the NEW DEFLECTION ANGLE FORMULA from formula.md, but this
model only sees AP/RIS geometry because UE coordinates are not part of the input.
...
INPUT FEATURES (~18 features: AP/RIS positions + derived geometry + link metrics):
  ...
  - AoA/AoD sines/cosines (azimuth incident/reflected)
  - AP→RIS offset (dx, dy, dz) plus azimuth/elevation sine-cosine pairs
