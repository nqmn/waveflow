...
source = np.array([8.0, 10.0, 0.0])    # Source position (height 0.45 m above RIS)
...
FoV_half = 30  # Field of view half-angle (FoV = 60 degrees)
...
theta_fill = np.linspace(math.radians(bisector - FoV_half),
                         math.radians(bisector + FoV_half), 200)
for sign in (+1, -1):
    edge_ang = bisector + sign * FoV_half
...
ax1.fill(x_fill, y_fill, color='gray', alpha=0.08, label=f'FoV ±{FoV_half}°')
...
# Plot 2: Quantized phase (1-bit) (bottom, full width)
ax2 = fig.add_subplot(gs[1, 0])
im2 = ax2.imshow(phase_quant_deg, cmap='bwr', origin='lower', vmin=0, vmax=180)
ax2.set_title('RIS 1-Bit Quantized Phase Map (5.8 GHz)', fontsize=12, fontweight='bold')
